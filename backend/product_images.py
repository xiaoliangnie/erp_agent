...
def resolve_product_image(product: dict, *, sku: str, style: str,
    ...
    local = _existing_image(CONFIG_IMAGE_DIR, sku, style)
    if local:
        return {"path": str(local), "source": "SKU 本地图片", "status": "ready", "error": ""}
    cached = _existing_image(cache_dir or ROOT / "data" / "product-images", sku, style)
    if cached:
...
class ProductImageService:
    ...
    def __init__(self, database_path: str | Path, image_dir: str | Path):
    ...
    def next(self, worker_id: str) -> dict | None:
        ...
        with self._lock, self._connect() as conn:
            ...
            if not row:
                return None
            conn.execute(
                "UPDATE product_image_jobs SET status='syncing',worker_id=?,updated_at=? WHERE id=?",
