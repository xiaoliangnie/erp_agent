...
换货不在网页执行：`/exchange` 已隐藏，旧路径重定向到工作台。鞋垫只在钉钉查询并回复「确认」后写入。
普通换货 dry-run / Worker 接口仍在（`/api/exchange/*`），无页面入口。试算和写入由后端
Playwright Digital Worker 打开已登录的聚水潭订单页，注入 `jst-order-exchange.core.js`，
调用页面原生 `_ACP('ChangeItem')`。
...
- 采购助手可以理解“把订单 A 的 SKU B 换成 SKU C”，以及“这批待发货异常单把 B 换成 C”：
  先按订单镜像收成明确 `o_id` 再生成待确认 dry-run。Agent 确认只登记试算；网页换货入口已隐藏，
  鞋垫真实写入只在钉钉确认。
- 鞋垫换货是单独一条固定流程：钉钉或对话里说「查询一下现在抖音需要更换的鞋垫订单」，
  ...
  发货中只列出不写。写入成功后一次连接按内部单号回写镜像，并把这批单记进已写入台账；增量同步还没跟上时，再查不会把同一批再列出来。
  同一会话重复「进行处理」会复用已有待确认，不另开一条。
  抖音鞋垫另有定时任务（默认开）：每天 09:30–18:30 每小时自动跑一轮，只处理抖音、不经确认；先发【开始执行】清单，写完再发总结。已跑过的档落盘，重启不重发「没有待处理」。有人待确认时空跑要写「人工待确认，本轮不抢」，不说成没有待处理。群里说「手动跑抖音鞋垫」立刻再跑一轮。ERP 被代发占用时本轮跳过，不空等。可用 `INSOLE_SCHEDULE_ENABLED=false` 关掉。
...
七个页面（同一个 React 单页应用；`/chat` `/workbench` `/exchange` 重定向到工作台）：

| 路由 | 看什么 | 日期口径 |
|---|---|---|
| `/dashboard` | 采购全景：金额、品类、尺码、入库进度 | `最早预计到货日期` |
| `/ledger` | 单号 / 日期 / 供应商 / 产品 / 交期 / 入库数量 / 采购员，按跟单三档催货 | `item_delivery_date`（交期） |
| `/spu` | 鞋服 SPU：缺货 / 断码 / 缺码、出库窗口、线上/线下、补货建议 | 出库 `io_date` 滚动窗口 |
| `/baihuo` | 自营百货：商品编码/名称、线上/线下销量、缺货 / 需补货 / 进货仓 | 同 `/spu` |
| `/purchase` | 从看板勾选商品，改数量后二次确认写入聚水潭采购单 | — |
| `/contract` | 选单、选票种和单价、预览、下载采购合同 | — |
| `/status` | 工作台：登录/花名、待办、数据源、服务、定时任务 | — |
...
访问 `http://127.0.0.1:8777/`（会跳到 `/dashboard`）。未登录只能看到登录页。页面只走同源 API，没有离线快照：
...
接口结果缓存 30 秒；过期后先返回上一份（10 分钟内）再后台刷新。服务自己续热本年缓存，同会话 25 秒内再进页不重拉，避免每次进看板都空等全年明细。
...
| 文件 | 作用 |
|---|---|
| `frontend/src/` | React 单页应用：十个页面、共用 API 客户端、payload 解码与设计令牌 |
...
揭收货未揭开时点内部单号（淘宝频控 / 解密超时 / 组件没挂上会区分）；其余收货人/手机/地址齐了再写「都齐了」。已填表不覆盖。
...
网页对话 `/chat` 已隐藏，问询和确认走钉钉。网页登录：钉钉绑定同意后私信花名和密码；
已绑定的人由管理员发「补发网页账号」（或 `scripts/run_dingtalk_cli.py issue-web`）。
忘记密码到群里发「绑定网页」重置。填到工作台，会话 30 天。只读查询不校验姓名；生成合同、换货、发催办必须能对上绑定表。
...
| 抖音鞋垫定时 | 工作流（不经工具确认） | 09:30–18:30 每小时自动跑；钉钉「手动跑抖音鞋垫」或 `scripts/run_insole_schedule.py` |
...
网页用登录后的 `X-Agent-Web-Token`。脚本仍可用 Bearer `AGENT_API_TOKEN`。未配置 Token 且无网页会话时写接口 401。

| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/api/agent/login` | `{username, password}` → `webToken` + 花名，会话 30 天 |
| GET | `/api/agent/me` | 当前网页会话 |
| POST | `/api/agent/logout` | 注销当前会话 |
| GET | `/api/agent/insole` | 鞋垫待处理 / 发货中清单 |
| POST | `/api/agent/insole/process` | `{oId}` 处理一单（点击即确认） |
...
正式口径只在 `docs/预测.md`：预测 ≠ 建议量 ≠ 下单；分位 `p10 / p50 / p90`；
`coverage_days` = `lead_time_days`；先销后采不走预测；库存缺失报错不用 0。
**LLM 只解释不改数字**，不抄 ERP「建议/本次采购数」。`service.py` 仍是旧实现；
旁路公式在 `backend/forecast/replenish.py`，未接线上。鞋服 SPU 总表走
`scripts/run_spu_alerts.py`（库存/出库已进 60 秒同步；`--inventory-history` 补不动销款），写入
`spu_style_snapshot` 并落 `files/outputs/spu/YYMMDD-鞋服SPU总表.xlsx`；看板 `/spu`。
自营百货圈定标签「自营百货」，结果表 `baihuo_style_snapshot`，看板 `/baihuo`。
日均改三窗折月：`月销量 = (7天×4 + 15天×2 + 30天) / 3`，`日均 = 月销量 / 30`。
看板看近 30 天趋势，另出 7 天 / 14 天 / 30 天件数。线上/线下按 7 / 15 / 30 天拆，店铺清单同这三个窗口。
行按**商品编码**（SKU）出，不用款式编码；名称用商品名称。
销量拆线上/线下：以 ERP 店铺设置分组为准（报表「销售渠道 → 按店铺设置中分组展示」的 `group_name`）；消防/渠道/公安/交警业务部、内部店铺算线下，其余线上。查不到分组时才退回店名规则。
日均和补货建议仍用合计销量。看板不展示断码/缺码，看缺货、需补货、进货仓待上架。
补货建议是 `日均 × 30 − 总库存`。建议下单 = 把补货建议向上取整（有起订量按起订量倍数，否则按 10；≤0 显示 —）。
分析用百货专属提示词，不套鞋服季节/断码话术。
鞋服日均公式不动，补货仍 `日均 × 60`；鞋服看板也拆 7/15/30 天线下对照。
`/spu` `/baihuo` 另出「进货仓」（ERP `in_qty`），不进总库存，只提示货已到进货仓、别按补货建议再下。
品名 / 商品名称后另有「供应商」列（最近一笔采购的简称，没有则 —），可按下拉筛选。
两页都可勾选款（或「勾选建议下单」）进 `/purchase`：鞋服同款列出该款式编码下全部商品编码，数量/单价/备注参考上次采购，没填数量的行不写入采购单。采购日期选年月日（写入 ERP 带上确认当下的时分秒）；采购员从历史署名下拉；税率随合同票种预选，可改。付款方式与合同同一套（3/7、发货前付款、到仓后付款、月度结算），写入 ERP 再映射 `payment_method`。确认页按聚水潭手工下单排，确认后 Playwright 调 `editor.aspx` 的 `_ACP('Confirm')` 写成待审核采购单；成功后再试生成采购合同（带上所选付款条款）。没有最近采购价就不填单价。
库存接口一 SKU 一行（即页面「实际库存数」，无分仓）；从未入库的 SKU 接口无行，按 0 计断码，
备注注明（库存表整体为空仍报错不出表）。鞋服圈定（2026-08-20）：三线 × 生产模式=自营，
带清仓品/淘汰品/有升级标签的整款剔除。列上删「25年净销量」，
加「前7天合计」「近7天合计」「周环比销量」=（近7天−前7天）/前7天
（前 7 天=昨天往回 8–14 天，为 0 记 "-"）。
不训生产模型、不注册 `forecast_sales`。

旧服务端实现（仅对照，不是 F0.1）仍是：
...
| 在途待入库 | 采购明细：数量 − 已入库，排除已取消 | 候选池已有，合格在途（ETA）未筛 |
| 可用库存 | 现势库存表（计划 `realtime_inventory`） | **下一步进镜像**，缺失时报错说明缺哪些 SKU，不用 0 兜底 |

销售表和库存表进实时库之前，订货建议不能当员工结论。
导出目录 `FORECAST_EXPORT_DIR`（默认 `D:\Predict_DATA`）在仓库外，**已清空**。
看板改套员工公式 Excel，不要往 `FORECAST_MODEL_DIR` 训占位 Baseline。
...
- `DINGTALK_REMINDER_ENABLED=true`：每天 `DINGTALK_REMINDER_TIME`（默认 08:30）发**跟单催办**。
  ...
  **谁的采购单只发一条给谁**。只私聊已绑定 userId 的人
  ...
  交期台账「发送提醒」走同一接口：确认弹窗后按当前采购员筛选和档位推送；须先登录，身份取网页会话花名。全量已推过后
...
- 「导出催办清单」按当前切片导出，多给两列：**交期来源**（交期 / 预计到货）和
  **下次提醒日**，直接拿去发
- 「发送提醒」把当前采购员/档位的需催清单**私聊**已绑定采购员（与定时催办同一口径，不发群）。需先登录；当日已成功推过同一批会提示「当日已推」
