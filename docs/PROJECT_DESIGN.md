...
采购合同以 `templates/采购合同模板.xlsx` 为业务和视觉基准。Agent、网页和命令行共用 `backend/contracts.py`：读取 ERP 单头/明细后，按供应商简称合并 `config/suppliers.json`，按 SKU/款号合并 `config/products.json`，再由 `backend/contract_workbook.py`（openpyxl）输出公式驱动的 Excel。
...
0. 商品资料与图片 URL 已通过 `items.query` 收进 `realtime_products`，供应商完整资料已进入
   ...
   **用主数据表兜底替换 `config/products.json` 的分类/包装/单位**（单价仍只认配置或 ERP）
   是 P2，**迁到专用机器之后再做**，见 `docs/MVP完善方案.md`。
1. 同步延迟与失败告警、操作日志已由 `scripts/health_watch.py` 和统一 `logging` 落地；
   数据质量检查仍待做。
2. 给催办动作增加独立状态表，记录每一波提醒的发送结果。
3. Agent 代码已落地、默认关闭。阶段 6 已接上黄金对话回放、网页 `staff_bindings` 校验、
   L0 `master_data_gaps`；催办推送可单独开，台账「发送提醒」走同一通道。对话不是主入口。
4. 部署时把数据库配置写入服务器密钥，不提交任何 env 凭证文件。
5. 合同选国标已接入：`/contract` 按目录族列出执行标准，选中值写入 `contract_line_gb` 并进 Excel「执行标准」列，候选项带状态角标。钉钉/网页助手用 L0 工具 `gb_catalog_status`、`lookup_gb_standards` 查目录水位和商品对应标准。每日同步成功后，合同已选用标准若变为现行或废止会推钉钉。目录同步仍见 `gb_standards.py`。
