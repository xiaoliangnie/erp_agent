...
| 项 | 内容 |
|----|------|
| 入口 | 采购单管理 → 新增采购单 → **手工下单** → `New('po')` |
| 路径 | `/app/scm/purchase/editor.aspx?type=po&IsSaveRemark=false&module=purchasemode` |
| 保存 | 页面 `Confirm()` → `_ACP('Confirm', cb, itemsJson, seller_id, fileList, modify_supplier)` |
| 探测 | 2026-08-21 只读打开编辑页，未在本轮点保存 |

**单头必填（页面 `head_must` / `CheckCondition`）：**

| 字段 | 说明 |
|------|------|
| `seller` / `seller_id` | 供应商名称与编号，缺编号不能保存 |
| `po_date` | 采购日期。ERP 打开页时已带当前时间；看板建单页面只选年月日，写入时拼成 `YYYY-MM-DD HH:mm:ss`（确认当下的时分秒） |
| `purchaser_name` | 采购员 |
| `wms_co_id` | 仓储方（本仓为 `0`） |
| `payment_method` | 聚水潭只认 `CurrentSettlement` / `CashOnDelivery` / `MonthlyStatement`。看板建单下拉复用合同 `payment_options`（3/7、发货前付款、到仓后付款、月度结算），写入时按 `erp_payment_defaults` 反查；未映射的条款（如 3/7）落 `CurrentSettlement` |
| 明细 | `sku_id`、`qty`、`price`；最多 1000 行 |

可选：到货日期、税率、运费、溢装比例、收货地址、备注、运营云仓、商品类型（默认成品）。`po_id` 保存后由 ERP 生成。本仓库走 `erp.create_purchase_order`，默认只保存、不「保存并审核生效」。
