...
> 本文是分阶段实现的依据；每个阶段落地后同步更新「实施路线」的状态。

## 1. 需求与子系统对应

| # | 需求 | 子系统 | 状态 |
|---|---|---|---|
| 1 | 用数据库历史数据训练预测模型，LLM 结合预测给订货建议 | `backend/forecast/` + Agent 工具 | **接口与链路已实现**；等销售/库存表与训练好的模型接入（见 `docs/预测模型接入.md`） |
...
现在 handler 把业务 dict 整份 `json.dumps` 进模型上下文，单条上限 20,000 字
（`runner.py` 的 `content[:20000]`）。`get_purchase_order` 会带全部明细，
`delivery_reminders` 默认 30 张单——一张单行数一多，模型就开始自己「汇总」数字，
正好违反「工具返回的数字不要再算一遍」。

给模型的返回统一成信封，**全量数据仍给页面和下载，不进 LLM**：
...
已确认：**员工权限一律相同，第一期不做角色矩阵**，确认动作谁发起谁确认。
...
FORECAST_MODEL_DIR=data/models         # 模型工件目录（gitignored）
```
