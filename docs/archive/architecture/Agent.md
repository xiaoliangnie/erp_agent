...
> **归档。** 现行说明见 [`../../架构.md`](../../架构.md) 与 [`../../开发.md`](../../开发.md)。
...
| 3 | 订单异常处理（SKU 换货） | `/exchange` + `backend/exchange/` 任务队列 + 后端 Playwright；对话可定位并登记 dry-run | 第一版已实现；缺的是「异常范围 → 明确 o_id 清单」的只读候选工具 |
...
- **不让模型写 ERP、不让模型开浏览器。** 换货 / 探测 / 搜 SKU / 商品图片由后端
  Playwright Digital Worker 执行类型化命令；没有开放写接口就不做「Agent 创建采购单」。
...
```text
 ...
 对话溢出：网页 /chat · 钉钉 Stream
  ...
  （只读）    forecast/     contracts.py exchange/
    │            │              │           │
 镜像库只读   模型工件        模板+config  后端 Playwright
 （hanli.env）                 主数据
                     │
...
也 **不直连聚水潭 OpenAPI**——写路径只有 Digital Worker 这一条，读路径只打本地镜像。
...
发给模型的上下文按固定顺序组装：系统规则与日期 → 渠道/角色/绑定范围 → 操作员记忆
（≤500 字，注入前扫描）→ 滚动摘要（≤800 字，已去掉金额/数量/日期）→ 当前业务对象
快照（只含采购单号 / `o_id` / SKU / 待确认编号）→ 本 epoch 近讯 → 本轮工具结果。
旧轮次工具结果压成短 JSON，需要时重查。`AGENT_ENABLED=false` 或未配置密钥时
`/api/agent/chat` 返回 503，与现有 `AGENT_API_TOKEN` 未配置的行为一致。

对话页已经把每步工具名和待确认卡片画出来——这是透明度要求，不要改成「只给一段自然语言」。

### 5.0 意图路由（先命令，后模型）

对照 DeepSeek Harness：用户命令在 `ctx.commands` 上注册，**不必开模型轮次**就能分派；
工具仍走 pre-execute → 权限 → execute → post-execute。对照 Grok Build：slash / skill
等特殊情况先处理，再进采样循环。本仓库不引入 Cordis、ACP、子 Agent 或 Code Mode。

`backend/agent/router.py` 是对外入口，`intents.py` 做抽槽和固定句。顺序是：

会话命令 → 实体/关键词规则 → 已注册工具或拒答/追问 → 未识别才进 LLM 工具循环。
**不另加一层 LLM Structured Output。** Intent Router 只回答「他想干什么」，
不接收角色、不查权限；`route_message(text, working_set=…)` 不接收角色。
当前话题快照里若只有一个采购单号 / `o_id` / SKU，缺槽追问会用它补上，不猜新编号。

输出统一为 `route` / `domain` / `operation` / `entities` / `missing_slots` /
`risk_level` / `confidence` / `calls`。route 现为
`exact_query` / `workflow` / `agent` / `knowledge` / `clarify` / `deny` / `command`。

固定句覆盖已注册工具：采购单、销售订单（明确 `o_id` 时可两步 L0）、催办、看板、
商品、国标、缺口、预测/订货建议（有 SKU 才调；分位数未冻结，不改入参）、
合同、代发、换货、鞋垫、品控登记/查询/日报。越界的正式建单、询价/RFQ/历史报价、
改地址、超卖直接 `deny`。品控关闭/撤销没有网页工具，追问去钉钉发原命令。
相邻意图（到货+出合同、逾期+鞋垫、代发+采购合同、缺价+出合同）先追问，不猜。
说不清的句子返回 `None`，再交给 LLM。禁止在路由里猜 `o_id` 或改工具返回的数字。
L1/L2 仍进 `pending_actions`。
...
| 订单异常 / 换货 | `/exchange` + 后端 Playwright 队列；对话可定位候选并登记 dry-run | 定位与话术可以走 LLM；筛单规则和 ERP 写入不行 |
...
`tests/fixtures/golden_dialogues.json`。每个已落地场景按 §38 补
**正常 / 省略参数 / 模糊 / 错误 / 相邻干扰**。CI 用假 LLM 跑 `GoldenReplayTests`，
同时 `IntentRouterTests` 断言这些原话先走确定性路由；真模型联调后再加夜间集。
...
**交互模型（已确认）：真实写入仍在换货页二次确认后由后端 Playwright Digital Worker 执行；
...
换货配置页（前端 /exchange 路由）
  商品数据选 源 SKU / 目标 SKU（或规则预设）+ 订单定位参数
        ...
        ▼
后端 Digital Worker（Playwright 注入同一段核心 JS，长轮询领任务）
...
- 比原 CLI 快在：去掉每次 osascript 进程往返和 0.5s 轮询，worker 在页内原生跑循环，
  进度直接回报服务端（Playwright 与 HTTP 同进程，不再经油猴）。
...
| 订单解析 / 试算 / 执行 | 后端 Playwright | 注入 `jst-order-exchange.core.js`，仍走页内 `_ACP` |
| 全自动（无人值守） | **鞋垫已走后端** | Playwright 只当浏览器宿主：`page.evaluate()` 注入同一段核心 JS，仍走 `_ACP`。写入后 `loadOrder` 回读 SKU，对不上不记成功。普通换货仍要 `/exchange` 二次确认 |

**「后台执行」的前提：** 鞋垫确认后由 `DigitalRuntime` 持有专用账号登录态（`erp-ai-state.json`），
不要求员工浏览器领取循环。普通换货、探测、搜 SKU、商品图片由同一 Worker 领取；`ERP_AI_ENABLED=false` 只作总闸，油猴不再领取。
写入证据落 `files/data/erp-evidence/`。
...
| 每天被提醒谁逾期 | 钉钉单聊已绑定采购员，定时任务，不经模型；未绑定不发群 |
| 看待确认 / 换货任务 / 品控待办并确认 | `/workbench`（与对话同一份 `pending_actions`） |
...
- **定时主动推送（已确认）**：每天早上按台账页同一套跟单三档（≤10 天 / ≤3 天 / 已逾期）
  生成催办清单，**按采购员拆开**：已绑定的发到与机器人的单聊，未绑定的仍发采购群。
  后续主数据缺口（供应商未维护、近期采购 SKU 无图）
...
已确认：第一期是 `viewer` / `operator` / `admin`，确认动作谁发起谁确认。
管理员私聊可用全部能力并审批绑定；员工私聊只确认任务。
**不按金额或「模型历史准确率」自动放行 L1/L2**——人少、主数据缺口大，误放行的成本高于多点一次确认。
viewer 只能跑 L0；拒绝时工具返回结构化 `permission`，不靠 prompt 拦。

`users` 是身份事实主体（`usr_...`）。`staff_bindings` 是兼容层：ERP 采购员姓名、网页署名、钉钉 userId
映射到同一个 `user_id`。同一人在 ERP 里可能同时有花名、真名（花名）和岗位署名
（如「利特」与「李佳冬（利特）」、「三三」与「鞋子理单（三三）」），
绑定任一已确认别名即可。未绑定的钉钉用户只读；
网页若只带共享 token、署了一个对不上的名字，L1/L2 直接拒绝。
详见 [`identity/用户身份.md`](../identity/用户身份.md)。

ERP 镜像库只读；禁止模型生成或执行任意 SQL，每项查询对应固定参数化工具。
专用 Digital Worker 的登录 cookie 只落本机 `files/data/secrets/erp-ai-state.json`，不进库、不进接口。
...
| `users` | 内部员工主表（`usr_`）；ERP 别名 JSON；钉钉字段预留 |
| `staff_bindings` | 采购员姓名 ↔ 钉钉 userId/手机号 ↔ `users.user_id`（兼容层；群内 @ 与 L1/L2 仍读这里） |
...
国标同步、换货、Agent、预测工件、钉钉 Stream / 催办、品控日报和代发调度状态。`scripts/health_watch.py`
...
- 钉钉：`dingtalk-stream` 已在 `requirements.txt`。Stream 对话要 `DINGTALK_ENABLED` +
  AppKey/Secret；催办可单独开。群内「绑定 姓名」提交申请，管理员私聊同意后写入 `staff_bindings`；
...
| 5. 换货接入 | 换货配置页 + SQLite 任务队列 + 后端 Playwright。组合筛选待销售订单库接入后开放 | **第一版已实现**；待在真实 ERP 测试环境完成一次人工确认验收 |
...
已敲定并移入正文：换货运行载体（后端 Playwright，§7）；催办推送形态（群内 @ 对应
