...
export function WorkbenchPanel() {
  const { credentials, noteAuthError, loggedIn } = useAuth();
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      setPayload(await agentApi.get<WorkbenchPayload>("/api/agent/workbench", credentialsRef.current));
      setMessage("");
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    load().catch((error: unknown) => setMessage(noteAuthError(error)));
  }, [load, loggedIn, noteAuthError]);
    ...
    <>
      <section className="panel">
        <div className="panel-head">
          <strong>待我处理</strong>
          <small>
            {loading ? "刷新中…" : `${confirmable.length} 条可处理 · 共 ${items.length} 条`}
          </small>
        </div>
        {message ? <div className="status error">{message}</div> : null}
        {!payload ? (
          <div className="empty">登录后加载待办</div>
        ) : items.length === 0 ? (
          <div className="empty">当前没有待办。钉钉里生成的确认会出现在这里。</div>
        ) : (
          items.map((item) => {
                      ...
                      <div className="action-row">
                        <button
                          ...
                          onClick={() => decideQuality(qualityIssueId(item), "resolve")
                            .catch((error: unknown) => setMessage(noteAuthError(error)))}
                        >
                          关闭
                        </button>
                        <button
